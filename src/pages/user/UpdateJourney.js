...
const UpdateJourney = () => {
  ...
  const [editorState, setEditorState] = useState(
    EditorState.createWithContent(
      ContentState.createFromBlockArray(
        convertFromHTML(detailPost?.description)
      )
    )
  );
  const [namePath, setNamePath] = useState("no files uploaded !");
  const [fileUpload, setFileUpload] = useState(detailPost?.image);
  ...
  const handlerUpdate = useMutation(async (e) => {
    try {
      e.preventDefault();
      ...
      const response = await API().put("/journey/" + detailPost?.id, config);
  ...
  return (
    <div className="page-newjourney">
      ...
      <div className="container form-input-newjourney">
        <form onSubmit={(e) => handlerUpdate.mutate(e)}>
          <input
            type="file"
            className="d-none"
            id="image"
